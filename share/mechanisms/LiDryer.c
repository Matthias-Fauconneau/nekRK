...
void fg_rates(const float log_T, const float T, const float T2, const float T4, const float rcp_T, const float rcp_T2, const float P0_RT, const float rcp_P0_RT,const float e_gibbs0_RT[], const float sc[], float* molar_rates) {
    float c, Pr, logFcent, logPr_c, f1;
    c = exp2(-12050.7 * rcp_T -0.406 * log_T + 31.724);
    const float cR0 = c * (sc[1]*sc[3] - e_gibbs0_RT[4]*e_gibbs0_RT[5]/(e_gibbs0_RT[1]*e_gibbs0_RT[3]) * sc[4]*sc[5]);
    c = exp2(-4566.49 * rcp_T + 2.67 * log_T -4.29903);
    const float cR1 = c * (sc[0]*sc[4] - e_gibbs0_RT[3]*e_gibbs0_RT[5]/(e_gibbs0_RT[0]*e_gibbs0_RT[4]) * sc[3]*sc[5]);
    c = exp2(-2490.15 * rcp_T + 1.51 * log_T + 7.75489);
    const float cR2 = c * (sc[0]*sc[5] - e_gibbs0_RT[2]*e_gibbs0_RT[3]/(e_gibbs0_RT[0]*e_gibbs0_RT[5]) * sc[2]*sc[3]);
    c = exp2(-9728.3 * rcp_T + 2.02 * log_T + 1.57046);
    const float cR3 = c * (sc[2]*sc[4] - e_gibbs0_RT[5]*e_gibbs0_RT[5]/(e_gibbs0_RT[2]*e_gibbs0_RT[4]) * sc[5]*sc[5]);
    c = exp2(-75779.1 * rcp_T -1.4 * log_T + 45.3795) * (2.5*sc[0] + sc[1] + 12*sc[2] + sc[3] + sc[4] + sc[5] + sc[6] + sc[7] + sc[8]);
    const float cR4 = c * (sc[0] - e_gibbs0_RT[3]*e_gibbs0_RT[3]/(e_gibbs0_RT[0])* rcp_P0_RT * sc[3]*sc[3]);
    c = exp2(-0 * rcp_T -0.5 * log_T + 12.5899) * (2.5*sc[0] + sc[1] + 12*sc[2] + sc[3] + sc[4] + sc[5] + sc[6] + sc[7] + sc[8]);
    const float cR5 = c * (sc[4]*sc[4] - e_gibbs0_RT[1]/(e_gibbs0_RT[4]*e_gibbs0_RT[4])* P0_RT * sc[1]);
    c = exp2(-0 * rcp_T -1 * log_T + 22.1685) * (2.5*sc[0] + sc[1] + 12*sc[2] + sc[3] + sc[4] + sc[5] + sc[6] + sc[7] + sc[8]);
    const float cR6 = c * (sc[3]*sc[4] - e_gibbs0_RT[5]/(e_gibbs0_RT[3]*e_gibbs0_RT[4])* P0_RT * sc[5]);
    c = exp2(-0 * rcp_T -2 * log_T + 35.1453) * (2.5*sc[0] + sc[1] + 12*sc[2] + sc[3] + sc[4] + sc[5] + sc[6] + sc[7] + sc[8]);
    const float cR7 = c * (sc[3]*sc[5] - e_gibbs0_RT[2]/(e_gibbs0_RT[3]*e_gibbs0_RT[5])* P0_RT * sc[2]);
    ...
    const float cR8 = c * (sc[1]*sc[3] - e_gibbs0_RT[6]/(e_gibbs0_RT[1]*e_gibbs0_RT[3])* P0_RT * sc[6]);
    c = exp2(-597.492 * rcp_T + 0 * log_T + 23.9847);
    const float cR9 = c * (sc[3]*sc[6] - e_gibbs0_RT[0]*e_gibbs0_RT[1]/(e_gibbs0_RT[3]*e_gibbs0_RT[6]) * sc[0]*sc[1]);
    c = exp2(-214.168 * rcp_T + 0 * log_T + 26.077);
    const float cR10 = c * (sc[3]*sc[6] - e_gibbs0_RT[5]*e_gibbs0_RT[5]/(e_gibbs0_RT[3]*e_gibbs0_RT[6]) * sc[5]*sc[5]);
    c = exp2(-0 * rcp_T + 0 * log_T + 24.9539);
    const float cR11 = c * (sc[4]*sc[6] - e_gibbs0_RT[1]*e_gibbs0_RT[5]/(e_gibbs0_RT[4]*e_gibbs0_RT[6]) * sc[1]*sc[5]);
    c = exp2(360.818 * rcp_T + 0 * log_T + 24.7846);
    const float cR12 = c * (sc[5]*sc[6] - e_gibbs0_RT[1]*e_gibbs0_RT[2]/(e_gibbs0_RT[5]*e_gibbs0_RT[6]) * sc[1]*sc[2]);
    c = exp2(-8698.84 * rcp_T + 0 * log_T + 28.6458);
    const float cR13 = c * (sc[6]*sc[6] - e_gibbs0_RT[1]*e_gibbs0_RT[7]/(e_gibbs0_RT[6]*e_gibbs0_RT[6]) * sc[1]*sc[7]);
    c = exp2(1182.86 * rcp_T + 0 * log_T + 16.9882);
    const float cR14 = c * (sc[6]*sc[6] - e_gibbs0_RT[1]*e_gibbs0_RT[7]/(e_gibbs0_RT[6]*e_gibbs0_RT[6]) * sc[1]*sc[7]);
    ...
    const float cR15 = c * (sc[7] - e_gibbs0_RT[5]*e_gibbs0_RT[5]/(e_gibbs0_RT[7])* rcp_P0_RT * sc[5]*sc[5]);
    c = exp2(-2882.19 * rcp_T + 0 * log_T + 24.5225);
    const float cR16 = c * (sc[3]*sc[7] - e_gibbs0_RT[2]*e_gibbs0_RT[5]/(e_gibbs0_RT[3]*e_gibbs0_RT[7]) * sc[2]*sc[5]);
    c = exp2(-5771.64 * rcp_T + 0 * log_T + 25.5225);
    const float cR17 = c * (sc[3]*sc[7] - e_gibbs0_RT[0]*e_gibbs0_RT[6]/(e_gibbs0_RT[3]*e_gibbs0_RT[7]) * sc[0]*sc[6]);
    c = exp2(-2882.19 * rcp_T + 2 * log_T + 3.2555);
    const float cR18 = c * (sc[4]*sc[7] - e_gibbs0_RT[5]*e_gibbs0_RT[6]/(e_gibbs0_RT[4]*e_gibbs0_RT[7]) * sc[5]*sc[6]);
    c = exp2(-0 * rcp_T + 0 * log_T + 19.9316);
    const float cR19 = c * (sc[5]*sc[7] - e_gibbs0_RT[2]*e_gibbs0_RT[6]/(e_gibbs0_RT[5]*e_gibbs0_RT[7]) * sc[2]*sc[6]);
    c = exp2(-6938.31 * rcp_T + 0 * log_T + 29.1115);
    const float cR20 = c * (sc[5]*sc[7] - e_gibbs0_RT[2]*e_gibbs0_RT[6]/(e_gibbs0_RT[5]*e_gibbs0_RT[7]) * sc[2]*sc[6]);
