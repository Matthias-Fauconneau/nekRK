...
void fg_rates(const dfloat log_T, const dfloat T, const dfloat T2, const dfloat T4, const dfloat rcp_T, const dfloat rcp_T2, const dfloat P0_RT, const dfloat rcp_P0_RT,const dfloat e_gibbs0_RT[], const dfloat sc[], dfloat* molar_rates) {
    dfloat c, Pr, logFcent, logPr_c, f1;
    c = exp2(-12050.7 * rcp_T -0.406 * log_T + 31.724);
    const dfloat cR0 = c * (sc[1]*sc[3] - e_gibbs0_RT[4]*e_gibbs0_RT[5]/(e_gibbs0_RT[1]*e_gibbs0_RT[3]) * sc[4]*sc[5]);
    c = exp2(-4566.49 * rcp_T + 2.67 * log_T -4.29903);
    const dfloat cR1 = c * (sc[0]*sc[4] - e_gibbs0_RT[3]*e_gibbs0_RT[5]/(e_gibbs0_RT[0]*e_gibbs0_RT[4]) * sc[3]*sc[5]);
    c = exp2(-2490.15 * rcp_T + 1.51 * log_T + 7.75489);
    const dfloat cR2 = c * (sc[0]*sc[5] - e_gibbs0_RT[2]*e_gibbs0_RT[3]/(e_gibbs0_RT[0]*e_gibbs0_RT[5]) * sc[2]*sc[3]);
    c = exp2(-9728.3 * rcp_T + 2.02 * log_T + 1.57046);
    const dfloat cR3 = c * (sc[2]*sc[4] - e_gibbs0_RT[5]*e_gibbs0_RT[5]/(e_gibbs0_RT[2]*e_gibbs0_RT[4]) * sc[5]*sc[5]);
    c = exp2(-75779.1 * rcp_T -1.4 * log_T + 45.3795) * (2.5*sc[0] + sc[1] + 12*sc[2] + sc[3] + sc[4] + sc[5] + sc[6] + sc[7] + sc[8]);
    const dfloat cR4 = c * (sc[0] - e_gibbs0_RT[3]*e_gibbs0_RT[3]/(e_gibbs0_RT[0])* rcp_P0_RT * sc[3]*sc[3]);
    c = exp2(-0 * rcp_T -0.5 * log_T + 12.5899) * (2.5*sc[0] + sc[1] + 12*sc[2] + sc[3] + sc[4] + sc[5] + sc[6] + sc[7] + sc[8]);
    const dfloat cR5 = c * (sc[4]*sc[4] - e_gibbs0_RT[1]/(e_gibbs0_RT[4]*e_gibbs0_RT[4])* P0_RT * sc[1]);
    c = exp2(-0 * rcp_T -1 * log_T + 22.1685) * (2.5*sc[0] + sc[1] + 12*sc[2] + sc[3] + sc[4] + sc[5] + sc[6] + sc[7] + sc[8]);
    const dfloat cR6 = c * (sc[3]*sc[4] - e_gibbs0_RT[5]/(e_gibbs0_RT[3]*e_gibbs0_RT[4])* P0_RT * sc[5]);
    c = exp2(-0 * rcp_T -2 * log_T + 35.1453) * (2.5*sc[0] + sc[1] + 12*sc[2] + sc[3] + sc[4] + sc[5] + sc[6] + sc[7] + sc[8]);
    const dfloat cR7 = c * (sc[3]*sc[5] - e_gibbs0_RT[2]/(e_gibbs0_RT[3]*e_gibbs0_RT[5])* P0_RT * sc[2]);
    Pr = exp2(-381.001 * rcp_T -1.72 * log_T + 29.2458) * (2*sc[0] + 0.78*sc[1] + 11*sc[2] + sc[3] + sc[4] + sc[5] + sc[6] + sc[7] + sc[8]);
    ...
    const dfloat cR8 = c * (sc[1]*sc[3] - e_gibbs0_RT[6]/(e_gibbs0_RT[1]*e_gibbs0_RT[3])* P0_RT * sc[6]);
    c = exp2(-597.492 * rcp_T + 0 * log_T + 23.9847);
    const dfloat cR9 = c * (sc[3]*sc[6] - e_gibbs0_RT[0]*e_gibbs0_RT[1]/(e_gibbs0_RT[3]*e_gibbs0_RT[6]) * sc[0]*sc[1]);
    c = exp2(-214.168 * rcp_T + 0 * log_T + 26.077);
    const dfloat cR10 = c * (sc[3]*sc[6] - e_gibbs0_RT[5]*e_gibbs0_RT[5]/(e_gibbs0_RT[3]*e_gibbs0_RT[6]) * sc[5]*sc[5]);
    c = exp2(-0 * rcp_T + 0 * log_T + 24.9539);
    const dfloat cR11 = c * (sc[4]*sc[6] - e_gibbs0_RT[1]*e_gibbs0_RT[5]/(e_gibbs0_RT[4]*e_gibbs0_RT[6]) * sc[1]*sc[5]);
    c = exp2(360.818 * rcp_T + 0 * log_T + 24.7846);
    const dfloat cR12 = c * (sc[5]*sc[6] - e_gibbs0_RT[1]*e_gibbs0_RT[2]/(e_gibbs0_RT[5]*e_gibbs0_RT[6]) * sc[1]*sc[2]);
    c = exp2(-8698.84 * rcp_T + 0 * log_T + 28.6458);
    const dfloat cR13 = c * (sc[6]*sc[6] - e_gibbs0_RT[1]*e_gibbs0_RT[7]/(e_gibbs0_RT[6]*e_gibbs0_RT[6]) * sc[1]*sc[7]);
    c = exp2(1182.86 * rcp_T + 0 * log_T + 16.9882);
    const dfloat cR14 = c * (sc[6]*sc[6] - e_gibbs0_RT[1]*e_gibbs0_RT[7]/(e_gibbs0_RT[6]*e_gibbs0_RT[6]) * sc[1]*sc[7]);
    Pr = exp2(-33032.7 * rcp_T + 0 * log_T + 36.8066) * (2.5*sc[0] + sc[1] + 12*sc[2] + sc[3] + sc[4] + sc[5] + sc[6] + sc[7] + sc[8]);
    ...
    const dfloat cR15 = c * (sc[7] - e_gibbs0_RT[5]*e_gibbs0_RT[5]/(e_gibbs0_RT[7])* rcp_P0_RT * sc[5]*sc[5]);
    c = exp2(-2882.19 * rcp_T + 0 * log_T + 24.5225);
    const dfloat cR16 = c * (sc[3]*sc[7] - e_gibbs0_RT[2]*e_gibbs0_RT[5]/(e_gibbs0_RT[3]*e_gibbs0_RT[7]) * sc[2]*sc[5]);
    c = exp2(-5771.64 * rcp_T + 0 * log_T + 25.5225);
    const dfloat cR17 = c * (sc[3]*sc[7] - e_gibbs0_RT[0]*e_gibbs0_RT[6]/(e_gibbs0_RT[3]*e_gibbs0_RT[7]) * sc[0]*sc[6]);
    c = exp2(-2882.19 * rcp_T + 2 * log_T + 3.2555);
    const dfloat cR18 = c * (sc[4]*sc[7] - e_gibbs0_RT[5]*e_gibbs0_RT[6]/(e_gibbs0_RT[4]*e_gibbs0_RT[7]) * sc[5]*sc[6]);
    c = exp2(-0 * rcp_T + 0 * log_T + 19.9316);
    const dfloat cR19 = c * (sc[5]*sc[7] - e_gibbs0_RT[2]*e_gibbs0_RT[6]/(e_gibbs0_RT[5]*e_gibbs0_RT[7]) * sc[2]*sc[6]);
    c = exp2(-6938.31 * rcp_T + 0 * log_T + 29.1115);
    const dfloat cR20 = c * (sc[5]*sc[7] - e_gibbs0_RT[2]*e_gibbs0_RT[6]/(e_gibbs0_RT[5]*e_gibbs0_RT[7]) * sc[2]*sc[6]);
