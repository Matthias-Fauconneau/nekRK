...
class CPickler(CMill):
        ...
        def viscosity_and_thermal_conductivity(self, mechanism, speciesTransport, do_declarations, NTFit):
                ...
                for transport_specie in speciesTransport:
                        ...
                        for n in range(NTFit):
                                ...
                                visc = (5.0 / 16.0) * np.sqrt(np.pi * molar_mass * kb * t / Na) / (self.om22(tr,dst) * np.pi * diameter**2)
                                #conductivity of spec at t
                                #eq. (30)
                                m_red = molar_mass / (2.0 * Na)
                                diffcoef = (3.0 / 16.0) * np.sqrt(2.0 * np.pi * kb**3 * t**3 / m_red) / (10.0 * np.pi * self.om11(tr,dst) * diameter**2)
                                #eq. (19)
        ...
        def binary_diffusion_coefficients(self, speciesTransport, do_declarations, NTFit) :
                ...
                for i, transport_specie_i in enumerate(specOrdered):
                        ...
                        for j, transport_specie_j in enumerate(specOrdered[0:i+1]):
                                ...
                                for n in range(NTFit):
                                    T = self.lowT + dt*n
                                    tr = T / epsm_k
                                    difcoeff = 3.0 / 16.0 * 1 / atmospheric_pressure * (np.sqrt(2.0 * np.pi * T**3 * kb**3 / m_red) / ( np.pi * sigma * sigma * self.om11(tr,dst)))
                                    tlog.append(np.log(T))
                                    #spdiffcoef.append(np.log(difcoeff))
                                    spdiffcoef.append(difcoeff / pow(T, 3./2.))
                                #
                ...
                self._indent()
                self._write('dfloat logT[3] = {log(T), pow(log(T), 2), pow(log(T), 3)};')
                self._write('dfloat T32 = pow(T, 3./2.);')
                for k,spec1 in enumerate(specOrdered):
                    ...
                    for j,spec2 in enumerate(specOrdered):
                        if k != j:
                            a = max(k, j)
                            b = min(k, j)
                            self._write('+ mole_fractions[%d] / (T32 * (%.8E + %.8E * logT[0] + %.8E * logT[1] + %.8E * logT[2]))' # Why is the coefficient order being reversed here ?
                                % (j, binary_diffusion_coefficients[a][b][0], binary_diffusion_coefficients[a][b][1], binary_diffusion_coefficients[a][b][2], binary_diffusion_coefficients[a][b][3]))
                    self._write(');')
                self._outdent()
                self._write('}')
                return
