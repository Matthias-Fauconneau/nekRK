...
cd $1
PYTHONPATH=$fuego:$fuego/fuego:$fuego/pyre:$fuego/journal:$fuego/weaver python2 $fuego/applications/fmc.py -mechanism=mechanism.inp -thermo=therm.dat -name=$output 1>/dev/null &&\
sed -i 's/+-/-/g' $output
sed -i 's/+ -/-/g' $output
sed -i 's/- -/+/g' $output
sed -i 's/,}/}/g' $output
