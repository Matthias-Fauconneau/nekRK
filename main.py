...
class Species:
    ...
    def transport_polynomials(self):
        T = linspace(300., 3000., 1) # breaks transport but speeds up testing the code generator for rates or pretty assembly code formatting
...
#ifdef __FG_ENABLE__
__FG_DEVICE__ dfloat sq(float x) {{ return x*x; }}
#define n_species {species.len}
#define n_active_species {len(active)}
__FG_CONST__ dfloat fg_molar_mass[{species.len}] = {{{', '.join([f'{w}' for w in species.molar_mass])}}};
__FG_CONST__ dfloat fg_rcp_molar_mass[{species.len}] = {{{', '.join([f'{1./w}' for w in species.molar_mass])}}};
...
__FG_DEVICE__ dfloat fg_viscosity_T_12(dfloat ln_T, dfloat ln_T_2, dfloat ln_T_3, const dfloat mole_fractions[]) {{
    {code([f'dfloat sqrt_viscosity_T14_{k} = {P[0]} + {P[1]}*ln_T + {P[2]}*ln_T_2 + {P[3]}*ln_T_3;' for k, P in enumerate(transport_polynomials.sqrt_viscosity_T14)])}
    {code([f'dfloat rcp_sqrt_viscosity_T14_{k} = 1./sqrt_viscosity_T14_{k};' for k in range(species.len)])}
 return
...
__FG_DEVICE__ dfloat fg_thermal_conductivity_T_12_2(dfloat ln_T, dfloat ln_T_2, dfloat ln_T_3, const dfloat mole_fractions[]) {{
    {code([f'dfloat conductivity_T12_{k} = {P[0]} + {P[1]}*ln_T + {P[2]}*ln_T_2 + {P[3]}*ln_T_3;' for k, P in enumerate(transport_polynomials.thermal_conductivity_T12)])}
 return (
...
__FG_DEVICE__ void fg_P_T_32_mixture_diffusion_coefficients(dfloat ln_T, dfloat ln_T_2, dfloat ln_T_3, const dfloat mole_fractions[], const dfloat mass_fractions[], dfloat* _) {{
 {code(f'''_[{k}] = (1. - mass_fractions[{k}]) / (
    {('+'+line).join([(lambda P: f"mole_fractions[{j}] / ({line}{P[0]} +{line} {P[1]}*ln_T +{line} {P[2]}*ln_T_2{line} + {P[3]}*ln_T_3{line})")(transport_polynomials.binary_thermal_diffusion_coefficients_T32[k if k>j else j][j if k>j else k]) for j in list(range(k))+list(range(k+1,species.len))])});''' for k in range(species.len))}
}}
#endif
